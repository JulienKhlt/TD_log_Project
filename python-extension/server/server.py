...
def _validate(ls, params):
    ...
    diagnostics = _validate_python(source) if source else []

    ls.publish_diagnostics(text_doc.uri, diagnostics)


def _validate_python(source):
    """Validates python file."""
    diagnostics = []
    return diagnostics
...
@python_server.feature(COMPLETION, trigger_characters=[','])
def completions(ls, params: CompletionParams = None):
    """Returns completion items."""
    text_doc = ls.workspace.get_document(params.textDocument.uri)
    lines = text_doc.lines
    ls.show_message_log(word(lines[0])[0])
    C = CompletionList(False, [
        CompletionItem('!'),
        CompletionItem('['),
        CompletionItem(']'),
        CompletionItem('{'),
    ...
    C.add_item(CompletionItem(lines[0]))
    C.add_item(CompletionItem(test()))
    return C
...
async def show_configuration_async(ls: PythonLanguageServer, *args):
    ...
    try:
        config = await ls.get_configuration_async(ConfigurationParams([
            ConfigurationItem('', PythonLanguageServer.CONFIGURATION_SECTION)
        ]))

        example_config = config[0].exampleConfiguration

        ls.show_message(
            'pythonServer.exampleConfiguration value: {}'.format(example_config)
        )
...
def show_configuration_callback(ls: PythonLanguageServer, *args):
    ...
    def _config_callback(config):
        try:
            ...
            ls.show_message(
                'pythonServer.exampleConfiguration value: {}'
